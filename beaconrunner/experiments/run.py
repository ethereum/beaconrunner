...
def run(executable=experiment, drop_network=False):
    ...
    try:
        state_update_blocks = executable.simulations[0].model.state_update_blocks
    except:
        state_update_blocks = executable.model.state_update_blocks

    df = post_process(df, state_update_blocks=state_update_blocks, parameters=parameters, drop_network=drop_network)
