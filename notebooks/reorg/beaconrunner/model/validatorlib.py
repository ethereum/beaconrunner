...
class BRValidator:
    ...
    def load_state(self, state: BeaconState, anchor_block: BeaconBlock) -> None:
        ...
        self.update_attester(current_state, self.data.current_epoch + 1)
            ...
            if self.data.head_root != head_root:
                ...
                if lca_epoch == current_epoch:
                    ...
                    if lca_epoch == current_epoch - 1:
                        ...
                        self.update_attester(current_state, current_epoch + 1)
                    self.update_sync_committee(current_state)
...
def get_proposer_slashings(validator, known_items):
    """
...
def slashable_propose(validator, known_items):
    ...
    slashable_signed_block = make_block(slot, get_ancestor(validator.store, head, slot-2), validator, processed_state, attestations)
